...
import { Plus, Database, RefreshCw, CheckCircle, AlertCircle, Trash2 } from 'lucide-react';
import { useGetGlobalCommentPoolStats, useAddGlobalComments } from '../../hooks/useQueries';
...
export default function AdminPoolManagement() {
  ...
  const { data: poolStats, isLoading: statsLoading } = useGetGlobalCommentPoolStats();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccessMsg(`Successfully added ${lines.length} comment${lines.length !== 1 ? 's' : ''} to the pool.`);
      setCommentInput('');
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Failed to add comments.';
      setErrorMsg(message);
    }
  };
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          {statsLoading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div className="text-center p-3 rounded-lg bg-muted/30">
                <div className="text-2xl font-bold text-foreground">{claimed}</div>
                <div className="text-xs text-muted-foreground mt-1">Claimed</div>
              </div>
              <div className="text-center p-3 rounded-lg bg-muted/30">
                <div className="text-2xl font-bold text-foreground">{total}</div>
                <div className="text-xs text-muted-foreground mt-1">Total Added</div>
              </div>
              <div className="text-center p-3 rounded-lg bg-muted/30">
      ...
      <Card className="space-card border-border/50">
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs">
            Enter one comment per line. All lines will be added to the global pool.
          </CardDescription>
        </CardHeader>
            ...
            >
              {addCommentsMutation.isPending ? (
                ...
                <>
                  ...
                  Add {lineCount > 0 ? `${lineCount} Comment${lineCount !== 1 ? 's' : ''}` : 'Comments'} to Pool
                </>
              )}
            </Button>
