...
import { Copy, Check, Zap, RefreshCw, PackageOpen } from "lucide-react";
import { useGetPoolStats, useGenerateBulkGlobal } from "../hooks/useQueries";
...
export default function BulkGeneratorView() {
  ...
  const isValidBatch =
    !isNaN(parsedBatchSize) && parsedBatchSize > 0 && parsedBatchSize <= 1000;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const raw = err instanceof Error ? err.message : "Failed to generate comments";
    ...
    <div className="max-w-2xl mx-auto space-y-6 p-4">
          ...
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground flex items-center justify-between">
              <span>
                ...
                <span className="ml-2 text-green-500 text-xs">✓ All fulfilled</span>
              </span>
              <Button
                variant="ghost"
                size="sm"
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-destructive">Unable to Generate Comments</DialogTitle>
