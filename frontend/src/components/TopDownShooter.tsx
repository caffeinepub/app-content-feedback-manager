...
export function TopDownShooter() {
  ...
  const stateRef = useRef({
    player: { x: CANVAS_W / 2, y: CANVAS_H / 2, vx: 0, vy: 0, radius: PLAYER_RADIUS },
    bullets: [] as Bullet[],
    enemies: [] as Enemy[],
    score: 0,
    time: 0,
  ...
  const spawnEnemy = useCallback(() => {
    ...
    let x = 0, y = 0;
    if (side === 0) { x = Math.random() * CANVAS_W; y = -ENEMY_RADIUS; }
    else if (side === 1) { x = CANVAS_W + ENEMY_RADIUS; y = Math.random() * CANVAS_H; }
    else if (side === 2) { x = Math.random() * CANVAS_W; y = CANVAS_H + ENEMY_RADIUS; }
    else { x = -ENEMY_RADIUS; y = Math.random() * CANVAS_H; }
    s.enemies.push({ x, y, vx: 0, vy: 0, radius: ENEMY_RADIUS, id: s.nextEnemyId++ });
  }, []);

  const resetGame = useCallback(() => {
    const s = stateRef.current;
    s.player = { x: CANVAS_W / 2, y: CANVAS_H / 2, vx: 0, vy: 0, radius: PLAYER_RADIUS };
    s.bullets = [];
    s.enemies = [];
    s.score = 0;
    s.time = 0;
  ...
  useEffect(() => {
    ...
    const loop = (timestamp: number) => {
      ...
      s.player.x = Math.max(PLAYER_RADIUS, Math.min(CANVAS_W - PLAYER_RADIUS, s.player.x + pvx * dt));
      s.player.y = Math.max(PLAYER_RADIUS, Math.min(CANVAS_H - PLAYER_RADIUS, s.player.y + pvy * dt));
  ...
  return (
    <div className="glass-card rounded-2xl p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="font-display font-bold text-xl neon-text">Top-Down Shooter</h3>
        <div className="flex gap-4 text-sm">
          <span className="text-neon-teal font-semibold">Score: {displayScore}</span>
      ...
      {!started ? (
        <div className="flex flex-col items-center justify-center py-8 space-y-4">
          <p className="text-muted-foreground text-sm text-center">
            Use WASD or Arrow keys to move. Click to shoot.<br />Survive as long as you can!
          </p>
          <Button onClick={resetGame} className="gradient-btn text-white font-semibold px-8">
        ...
        <div className="relative">
          ...
          {gameOver && (
            <div className="absolute inset-0 flex flex-col items-center justify-center rounded-xl"
              ...
              <h3 className="text-3xl font-display font-bold text-destructive mb-2">Game Over!</h3>
              <p className="text-foreground mb-1">Score: <span className="neon-text font-bold">{displayScore}</span></p>
              <p className="text-muted-foreground mb-6">Survived: {displayTime}s</p>
              <Button onClick={resetGame} className="gradient-btn text-white font-semibold">
